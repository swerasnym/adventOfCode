...
star1(Data) ->   
    Cards = 10007,
    F =   lists:foldl(fun (Command, Function) -> instruction3(Command,Function,Cards) end, {1, x, 0}, Data),
    eval(F, 2019, Cards).
...
instruction("deal with increment " ++ NumberStr, Deck) ->
    Size = length(Deck),
    Number = list_to_integer(NumberStr),
    Order = [{Pos rem Size, Card} || {Pos, Card} <- lists:zip(lists:seq(0,Number*Size-1,Number), Deck)],
