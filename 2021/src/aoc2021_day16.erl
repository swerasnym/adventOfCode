...
star1(Data) ->
    ...
    {Data, sum_versions(Parsed), Parsed}.
...
parse_message(Message) ->
 ...
 %%   io:format("~p~n", [{Message, Verssion, Type}]),
    ...
    {{{ver, list_to_integer(Verssion, 2)}, {type, Type}, Data}, {rest, Rest}} .

parse_message("100", Message) -> %% integer
    parse_value(Message, []);
...
%%	    io:format("~p~n", [{LengthB, Message1}]),
	    ...
	    {parse_message(SubPacket), {rest, Rest}};    
	$1 ->
...
sum_versions({Data, {rest, Rest}}) ->
     {Parsed, _} = parse_message(Rest),
    sum_versions(Data) + sum_versions(Parsed);
sum_versions({{ver, V}, T, Data})->
    V + sum_versions(Data);
sum_versions([{{ver, V}, T, Data}| Rest]) ->
