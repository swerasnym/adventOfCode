...
on(List) ->
    lists:filter(fun({S, _, _, _}) -> S == on end, List).
...
split_x(0, {_, X, _, _} = C1, {_, X, _, _} = C2) ->
    {[C1, C2], []};
...
split_y(0, {_, _, Y, _} = C1, {_, _, Y, _} = C2) ->
    {[C1, C2], []};
...
split_z(0, {_, _, _, Z} = C1, {_, _, _, Z} = C2) ->
    {[C1, C2], []};
