...
%% would mind if you made the system temporarily ignore missing cid
...
star1(Data) ->
    count([validate(Passport)  || Passport <- Data]).

star2(Data) ->
    count([validate2(Passport)  || Passport <- Data]).


read(File) ->
    {ok, Bin} = file:read_file(File),
    [
     to_map(string:split(string:replace(Pass, "\n", " ", all)," ",all))

     || Pass <-string:split(string:trim(binary_to_list(Bin)), "\n\n", all)].
...
validate(#{byr := _Byr, iyr:= _Iyr, eyr := _Eyr, hgt := _Hgt,  hcl:= _Hcl, ecl:=_Ecl, pid:=_Pid  } ) ->
    ok;
validate(_) ->
    nok.

validate2(#{byr := Byr, iyr:= Iyr, eyr := Eyr, hgt := Hgt,  hcl:=Hcl, ecl:=Ecl, pid:=Pid  } ) ->
