...
count_mosters([Map = #{dim := {Xdim, Ydim}} | Maps], Monster = #{dim := {Mx, My}}) ->
    Positions =
        [{X, Y}
         || X <- lists:seq(0, Xdim - Mx - 1),
            Y <- lists:seq(0, Ydim - My - 1),
...
commbind(Grid = #{{0, 0} := #{dim := {Xdim, Ydim}}}) ->
    List =
        [{{Gx * (Xdim - 2) + X - 1, Gy * (Ydim - 2) + Y - 1}, Value}
...
get_tansforms(Map) ->
    [Map,
     rotate(Map),
     rotate(rotate(Map)),
     rotate(rotate(rotate(Map))),
     flip_x(Map),
     rotate(flip_x(Map)),
     rotate(rotate(flip_x(Map))),
     rotate(rotate(rotate(flip_x(Map))))].
...
rotate(Map = #{dim := Dim = {Xdim, Ydim}}) ->
    ...
    B = Ydim div 2,
    NewMap =
        maps:from_list([{{-(Y - B) + A - 1, X}, Value} || {{X, Y}, Value} <- maps:to_list(Map)]),
    NewMap#{dim => Dim}.

flip_x(Map = #{dim := Dim = {Xdim, _Ydim}}) ->
    NewMap =
        maps:from_list([{{Xdim - 1 - X, Y}, Value} || {{X, Y}, Value} <- maps:to_list(Map)]),
    NewMap#{dim => Dim}.
...
get_edges(ImageMap = #{dim := {Xdim, Ydim}}) ->
    #{top => list_to_integer([maps:get({X, 0}, ImageMap) || X <- lists:seq(0, Xdim - 1)], 2),
      topRev =>
          list_to_integer([maps:get({X, 0}, ImageMap) || X <- lists:seq(Xdim - 1, 0, -1)], 2),
      bot =>
          list_to_integer([maps:get({X, Ydim - 1}, ImageMap) || X <- lists:seq(0, Xdim - 1)], 2),
      botRev =>
          list_to_integer([maps:get({X, Ydim - 1}, ImageMap) || X <- lists:seq(Xdim - 1, 0, -1)],
                          2),
      left => list_to_integer([maps:get({0, Y}, ImageMap) || Y <- lists:seq(0, Ydim - 1)], 2),
      leftRev =>
          list_to_integer([maps:get({0, Y}, ImageMap) || Y <- lists:seq(Ydim - 1, 0, -1)], 2),
      right =>
          list_to_integer([maps:get({Xdim - 1, Y}, ImageMap) || Y <- lists:seq(0, Ydim - 1)], 2),
      rightRev =>
          list_to_integer([maps:get({Xdim - 1, Y}, ImageMap) || Y <- lists:seq(Ydim - 1, 0, -1)],
                          2)}.
